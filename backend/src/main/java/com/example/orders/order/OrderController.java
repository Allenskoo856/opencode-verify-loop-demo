...
public class OrderController {
  ...
  @PostMapping @ResponseStatus(HttpStatus.CREATED) public Response create(org.springframework.security.core.Authentication a, @Valid @RequestBody CreateRequest request) { try { return new Response(service.create(user(a), request.title)); } catch (IllegalArgumentException e) { throw new ResponseStatusException(HttpStatus.BAD_REQUEST, e.getMessage()); } }
