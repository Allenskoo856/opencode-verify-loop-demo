...
发布包包含前端、后端、PostgreSQL 和 Verify Controller 的 Linux AMD64 镜像归档，静态 Go `verify-loop`、编译后的 Node `verify-loop.js`、离线 Compose、策略/Gate、`acceptance` 契约与项目适配清单、环境模板和 `SHA256SUMS`。OpenCode CLI、模型服务、源码依赖缓存和企业凭据不包含在介质内；Skills、插件和手册随 Git 仓库交付，需一并带入内网或使用仓库压缩包。
...
node verify-controller-ts/dist/verify-loop.js accept --spec acceptance/specs/orders-api.json  # 服务启动后执行真实 API 契约
```
